...
(function(){
  var ldcvNodes, ldrsNodes, ldrs1, ldrs2, ldrs1Demo, kit, ldsel, ldselNodes, threeInit, threeRoot, ref$, camera, scene, renderer, w, h, controls, d, shape, geom, mat, mesh, group, light;
  ...
  window.ldsel = ldsel = new ldSelect({
    root: '.ldsel'
  });
  ldselNodes = ld$.find(document, '#ldsel-animate i');
  window.ldselAnimate = function(node){
    return ldsel.get({
      host: node
    }).then(function(n){
      var k;
      ...
      return ldselNodes.map(function(d, i){
        var delay;
        delay = i / ldselNodes.length;
        if (!k.config.repeat) {
          delay = -1 + delay;
        }
        k.animateJs(d, 0);
